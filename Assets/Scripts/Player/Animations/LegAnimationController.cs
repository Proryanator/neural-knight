...
namespace Player.Animations{
	...
	public class LegAnimationController : MonoBehaviour{
		...
		[SerializeField]
		private float _legRotationSpeed = 1f;

		[Tooltip("If true, draws a debug line in the direction of the legs facing direction.")] [SerializeField]
		...
		[Tooltip("The maximum angle difference between your movement direction and facing direction.")] [SerializeField]
		private float _turnAngleThreshold = 30;
		...
		private void GetMovementInfo(){
			...
			_rotationTowardsFacingDirection =
				GetRotationWithStartingDirection(GetAngle(_facingDirection), _spriteFacingDirection);
			_rotationTowardsMovement =
				GetRotationWithStartingDirection(GetAngle(_moveDirection), _spriteFacingDirection);
		}
		...
		private void RotateLegsTowardMovementDirection(){
			...
			if (_moveDirection.Equals(Vector2.zero) ||
			    _rotationTowardsMovement.Equals(_rotationTowardsFacingDirection)){
				...
				transform.rotation = Quaternion.Lerp(transform.rotation, newRotation,
					Time.fixedDeltaTime * _legRotationSpeed);
			}

			if (_drawDebugLines){
				// draw the facing direction's line
				Debug.DrawLine(transform.position,
					(Vector2) transform.position + (_faceMouseController.GetFacingDirection().normalized * 5),
					Color.green);
				// draw the movement direction's line
				Debug.DrawLine(transform.position, (Vector2) transform.position + (_moveDirection.normalized * 5),
					Color.blue);
				// draw the facing direction of the legs
				Debug.DrawLine(transform.position, (Vector2) transform.position + ((Vector2) transform.up * 5),
					Color.red);
			}
		}
		...
		private Quaternion GetBackwardAngle(){
			float absoluteAngleBetween =
				Mathf.Abs(Quaternion.Angle(_rotationTowardsMovement, _rotationTowardsFacingDirection));
			...
			if (absoluteAngleBetween > 90){
				...
				return GetRotationWithStartingDirection(_rotationTowardsMovement.eulerAngles.z + 270,
					_spriteFacingDirection);
			}
