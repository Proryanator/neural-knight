...
public class MoveToCenterMovementPattern : AbstractAIMovementPattern{
	...
	private void Awake(){
		...
		if (_mapCentralPoint == null){
			Debug.LogWarning("There was no object set with the tag: " + AllTags.MAP_CENTRAL_POINT + " and movement towards center will not work.");
		}
	}

	public override void Move(){
		if (_mapCentralPoint != null){
			transform.position = Vector2.MoveTowards(transform.position, (Vector2)_mapCentralPoint.transform.position, _moveSpeed * Time.deltaTime);
		}
	}
}